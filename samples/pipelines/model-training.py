...
def download_data_op(pvc_name,
                       pvolumes, step_name='download_data'):
    return dsl.ContainerOp(
        name=step_name,
        image='ubuntu',
        command=["/bin/sh", "-c"],
        arguments=[
            "apt update && apt install -y git && git clone -b 0.1.25 https://github.com/kubeflow/pipelines && cp -av /pipelines/samples/tfx/taxi-cab-classification /mnt/",
        ],
        pvolumes=pvolumes
    )
...
def taxi_cab_classification(
    ...
    vop = dsl.VolumeOp(
        name='create-volume',
        resource_name='taxicabvol',
        size='3Gi'
    )

    dldata = download_data_op(
        vop.output,
        {'/mnt': vop.volume}
    )
